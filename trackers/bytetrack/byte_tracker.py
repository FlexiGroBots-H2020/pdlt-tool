...
from smoothers.Smoothers import Smoother

class STrack(BaseTrack):
    shared_kalman = KalmanFilter()
    def __init__(self, tlwh, score, cls, depth):
        ...
        self.cls = cls
        # Depth variable
        self.depth_kf = Smoother(5) # set the number of previous frames to have into account
        self.depth = depth
    ...
    def update(self, new_track, frame_id):
        ...
        self.depth = self.depth_kf.smooth_value(new_track.depth)
...
class BYTETracker(object):
    ...
    def update(self, dets, _, depths):
        ...
        if len(dets) > 0:
            '''Detections'''
            detections = [STrack(xyxy, s, c, d) for 
                (xyxy, s, c, d) in zip(dets, scores_keep, clss_keep, depths_keep)]
        else:
            detections = []
        ...
        if len(dets_second) > 0:
            '''Detections'''
            detections_second = [STrack(xywh, s, c, d) for (xywh, s, c, d) in zip(dets_second, scores_second, clss_second, depths_second)]
        else:
            detections_second = []
