...
INFTY_COST = torch.tensor(100000., device="cuda:0" if torch.cuda.is_available() else "cpu")
...
def gate_cost_matrix(
    ...
    measurements = torch.stack(
        [detections[i].to_xyah() for i in detection_indices],dim=0)
    for row, track_idx in enumerate(track_indices):
        track = tracks[track_idx]
        gating_distance = track.kf.gating_distance(track.mean, track.covariance, measurements, only_position)
        cost_matrix[row].where (gating_distance > gating_threshold,gated_cost)
