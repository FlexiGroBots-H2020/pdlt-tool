...
class Track:
    ...
    def __init__(self, detection, track_id, class_id, conf, n_init, max_age, ema_alpha,
        ...
        if feature is not None:
            feature /= torch.linalg.norm(feature)
    ...
    def ECC(self, src, dst, warp_mode = cv2.MOTION_EUCLIDEAN, eps = 1e-5,
        ...
        if warp_mode == cv2.MOTION_HOMOGRAPHY :
            warp_matrix = torch.eye(3, 3, dtype=torch.float32,device=self.device)
        else :
            warp_matrix = torch.eye(2, 3, dtype=torch.float32,device=self.device)
    ...
    def camera_update(self, previous_frame, next_frame):
        ...
        if warp_matrix is None and src_aligned is None:
            return
        [a,b] = warp_matrix
        warp_matrix=torch.cuda.FloatTensor([a,b,[0,0,1]])
        warp_matrix = warp_matrix.tolist()
        matrix = self.get_matrix(warp_matrix)

        x1, y1, x2, y2 = self.to_tlbr()
        x1_, y1_, _ = matrix @ torch.cuda.FloatTensor([x1, y1, 1]).T
        x2_, y2_, _ = matrix @ torch.cuda.FloatTensor([x2, y2, 1]).T
        w, h = x2_ - x1_, y2_ - y1_
