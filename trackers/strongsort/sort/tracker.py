...
class Tracker:
    ...
    """
    GATING_THRESHOLD = torch.sqrt(
        torch.tensor(kalman_filter.chi2inv95[4], device="cuda:0" if torch.cuda.is_available() else "cpu"))
    ...
    def _full_cost_metric(self, tracks, dets, track_indices, detection_indices):
        ...
        pos_cost = torch.empty([len(track_indices), len(detection_indices)], device=self.device)
        msrs = torch.stack([dets[i].to_xyah() for i in detection_indices], dim=0)
        for row, track_idx in enumerate(track_indices):
            pos_cost[row, :] = torch.sqrt(
                self.kf.gating_distance(
                    tracks[track_idx].mean, tracks[track_idx].covariance, msrs, False
                )
            ) / self.GATING_THRESHOLD
        pos_gate = torch.gt(pos_cost, 1.0)
        # Now Compute the Appearance-based Cost Matrix
        app_cost = self.metric.distance(
            torch.stack([dets[i].feature for i in detection_indices], dim=0),
            [tracks[i].track_id for i in track_indices],
        )
        app_gate = torch.gt(app_cost, self.metric.matching_threshold)
        # Now combine and threshold
        cost_matrix = self._lambda * pos_cost + (1 - self._lambda) * app_cost
        cost_matrix[torch.logical_or(pos_gate, app_gate)] = linear_assignment.INFTY_COST
        # Return Matrix
        return cost_matrix
