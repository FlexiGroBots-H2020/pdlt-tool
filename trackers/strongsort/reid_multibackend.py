...
class ReIDDetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='osnet_x0_25_msmt17.pt', device=torch.device('cuda:0'), fp16=False):
        super().__init__()
    ...
    def _preprocess(self, im_batch):
        ...
        for element in im_batch:
            # element=torch.transpose(element,2,0)
    ...
    def forward(self, im_batch):
        ...
        # JR; to process it faster
        # if isinstance(im_batch, torch.Tensor):
        im_batch = [i.permute(2, 0, 1).div(255) for i in im_batch ]
        # JR: end
