...
class Mplot2d:
    ...
    def enqueue_save_figure(self, filename, avg_focal_length=None):
        self.queue.put(('save', filename, avg_focal_length))
    ...
    def drawer_refresh(self, queue, lock):            
        while not queue.empty():      
            ...
            if item[0] == 'save':
                # Save command detected, now expecting the item to also include avg_focal_length
                _, filename, avg_focal_length = item
                self.save_figure(filename, lock, avg_focal_length)  # Pass avg_focal_length to save_figure method
                continue

            
            # Regular data processing
            xy_signal, name, color, marker, linestyle, linewidth = item
            if name in self.handle_map:
                ...
                handle, = self.ax.plot(xy_signal[0], xy_signal[1], c=color, marker=marker, label=name, linestyle=linestyle, linewidth=linewidth)    
                self.handle_map[name] = handle  

        if self.got_data:                 
            self.plot_refresh(lock)

    def save_figure(self, filename, lock, avg_focal_length=None):
        lock.acquire()
        try:
            if avg_focal_length is not None:
                # Draw a dashed horizontal line at the average focal length
                self.ax.axhline(y=avg_focal_length, color='r', linestyle='--', label=f'Avg Focal: {avg_focal_length:.2f}')
                self.ax.legend()  # Display the legend with average focal length
    ...
    def draw(self, xy_signal, name, color='r', marker='.', linestyle='-', linewidth=1):    
        if self.queue is None:
            return
        self.queue.put((xy_signal, name, color, marker, linestyle, linewidth))
